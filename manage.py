...
def index():
    ...
    # 遍历文章数据
    return render_template('home.html', articles=articles, page=int(page), len=len(articles)) # 渲染模板
...
def register():
    ...
    if request.method == "POST" and form.validate_on_submit():
        ...
        if is_mull(username=username, password=password, confirm=confirm):
            if checkusername(username=username):
                password = hash(password)
                addacount(username=username, password=password, email=email)
...
def login():
    ...
    if form.validate_on_submit():   # 如果提交表单，并字段验证通过
        ...
        if hashc(password_candidate, password):
            # 写入session
